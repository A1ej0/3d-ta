...
export default function PriceSummary({
  ...
  // If user profile has an address, pre-fill it
  useEffect(() => {
  ...
  const handleCheckout = async () => {
    ...
    if (shippingMethod !== "recogida" && !shippingAddress.trim()) {
      ...
      <div className="glass rounded-xl p-5 space-y-4">
        ...
        {user ? (
          <div className="space-y-4">
            ...
            {shippingMethod !== "recogida" && (
              <div className="space-y-1.5 animate-slide-up">
                <Label htmlFor="shipping-address" className="text-xs">Dirección de Envío *</Label>
                <Textarea
                  id="shipping-address"
          ...
          <div className="text-center py-4 space-y-3">
            <p className="text-sm text-muted-foreground">Debes iniciar sesión para realizar la orden.</p>
          </div>
        )}
      </div>
